(() => {
  ...
  const MATH_INLINE_PATTERN = /f′?\(x\)\s*=\s*[−-]?(?:√(?:\([^()]+\)|[A-Za-z]|\d+(?:[,.]\d+)?)|\d*π|\d+(?:[,.]\d+)?|[a-zω])\s*\/\s*(?:\([^()]+\)(?:²|³)?|\d+(?:[,.]\d+)?|[a-zω])|P\([^()]*\)\s*\/\s*P\([^()]*\)|(?:sin|cos)\([^()]+\)\s*\/\s*[a-zω]|norm\(vec\([^)]+\)\)|vec\([^)]+\)\s*·\s*vec\([^)]+\)|\(\s*vec\([^)]+\)\s*,\s*vec\([^)]+\)\s*\)|vec\([^)]+\)|(?:√?\d+|ρ)\((?:cos|sin)\([^()]+\)\s*\+\s*(?:sin|cos)\([^()]+\)i\)|\[(?:√?\d+|ρ)\s*,\s*[^,\]]+\]|(?:cos|sin)\([^()]+\)|P\([^()]*\)(?:\s*=\s*[−-]?\d+(?:[,.]\d+)?)?|[A-Z]\s*=\s*\{[^{}]+\}|[A-Z]\s*[∪∩]\s*[A-Z]|u[₀₁₂₃₄₅₆₇₈₉₊₋ₙ]+(?:\s*=\s*(?:u[₀₁₂₃₄₅₆₇₈₉₊₋ₙ]+|[−-]?\d+)(?:\s*[+−-]\s*\d+)?)?|(?:[−-]?\d*)?\(x\s*[+−-]\s*\d+\)(?:\(x\s*[+−-]\s*\d+\))+(?:\s*=\s*0)?|[xy]\s*=\s*[−-]?(?:\d+(?:[,.]\d+)?)?x(?:\s*[+−-]\s*\d+(?:[,.]\d+)?)?|(?:f′?\(x\)|[xy])\s*[=<>⩽⩾]\s*[−-]?\d+(?:\s+(?:ou|et)\s*[xy]\s*[=<>⩽⩾]\s*[−-]?\d+)?|\d+\s*×\s*10[⁻⁰¹²³⁴⁵⁶⁷⁸⁹]+/g;
  ...
  function appendMathCharacters(target, text) {
    String(text)
      ...
      if (fragment === "∪" || fragment === "∩") {
        ...
        operator.className = "math-set-operator";
        operator.textContent = fragment;
